...
class BaseColumnEnsembleRegressor(_HeterogenousMetaEstimator, BaseRegressor):
    ...
    def __init__(self, estimators, verbose=False):
        ...
        self.remainder = "drop"
        super(BaseColumnEnsembleRegressor, self).__init__()
    ...
    def _fit(self, X, y):
        if self.estimators is None or len(self.estimators) == 0:
            ...
            self.estimators = [
                (self.estimators[0][0] + str(i), self.estimators[0][1], i)
        ...
        for name, estimator, column in self._iter(replace_strings=True):
            ...
            estimator.fit(_get_column(X, column), y)
            estimators_.append((name, estimator, column))

        self.estimators_ = estimators_
        return self

    def _collect_preds(self, X):
        return np.asarray(
            [
                estimator.predict(_get_column(X, column))
                for (name, estimator, column) in self._iter(replace_strings=True)
            ]
        )

    def _predict(self, X) -> np.ndarray:
        return np.average(self._collect_preds(X), axis=0)


class ColumnEnsembleRegressor(BaseColumnEnsembleRegressor):
    ...
    def __init__(self, estimators, remainder="drop", verbose=False):
        ...
        super(ColumnEnsembleRegressor, self).__init__(estimators, verbose=verbose)
